...
from persons_msgs.msg import Person
...
class Prediction():
    def __init__(self):
        ...
        self.ksub = rospy.Subscriber('/argon/kinectPerson', Person, self.callback)
    ...
    def callback(self, msg):
        ...
        rospy.loginfo("now recog human:%s",str(len(msg.bodys)))

        msgs = MarkerArray()
        for u, body in enumerate(msg.bodys):

            color = 2
            if body.is_speaked == True:
                color = 1
                
            if body.is_tracked == False:
                continue
            # ---points---
            points = []
