...
public:
  ...
  vector<geometry_msgs::Quaternion> qt_data;
  double time;
  double sec;
  bool speaked;
...
public:
	...
	for(int i=0; i<jit->second.size(); ++i)
	  {
		...
		picojson::array qqcol;
		qqcol.push_back(picojson::value(jit->second[i].qt_data[jj].x));
		qqcol.push_back(picojson::value(jit->second[i].qt_data[jj].y));
		qqcol.push_back(picojson::value(jit->second[i].qt_data[jj].z));
		qqcol.push_back(picojson::value(jit->second[i].qt_data[jj].w));
		qcol.push_back(picojson::value(qqcol));

	      }

	    picojson::object obj;
	    obj["joints"] = picojson::value( jcol );
	    obj["orientation"] = picojson::value( qcol );
	...
	if( msg->human[i].body.joints.size() )
	  {
	    ...
	    vector<geometry_msgs::Quaternion> orientations;
	                         	    
	    //関節の三次元位置だけ取得し蓄積
	    joints = pnt_store( msg->human[i].body.joints );
	    orientations = qt_store( msg->human[i].body.joints );

	    JData jdata;
	    jdata.data = joints;
	    jdata.qt_data = orientations;
  ...
  vector<geometry_msgs::Quaternion> qt_store(vector<humans_msgs::Joints> jt)
  {
    vector<geometry_msgs::Quaternion> qts;
    for(int i=0; i<jt.size(); ++i)
      {
	qts.push_back( jt[i].orientation );
      }
    return qts;
  }
...
{
  ros::init(argc, argv, "save_kinect_orientation");
  MyClass mc;
  ros::spin();
  return 0;
}
