...
#include "humans_msgs/AudioData.h"
...
private:
  ...
  typedef message_filters::Subscriber< humans_msgs::AudioData > AudioSubscriber;
  AudioSubscriber audio1_sub_;
  AudioSubscriber audio2_sub_;
  
  
  typedef message_filters::sync_policies::ApproximateTime<
    humans_msgs::Humans, humans_msgs::Humans, humans_msgs::AudioData, humans_msgs::AudioData
    > MySyncPolicy;
...
public:
  MyClass()
    :pnh("~"),
     ...
     audio1_sub_( nh, "/humans/audio/1", 100 ),
     audio2_sub_( nh, "/humans/audio/2", 100 ),     
     sync( MySyncPolicy( 10 ), kinect1_sub_, kinect2_sub_, audio1_sub_, audio2_sub_ )
  {

    sync.registerCallback( boost::bind( &MyClass::callback, this, _1, _2, _3, _4 ) );
  ...
  void callback(
		...
		const humans_msgs::AudioData::ConstPtr& msg3,
		const humans_msgs::AudioData::ConstPtr& msg4)
  {
    double now_sec = ros::Time::now().toSec();


    cout << "msg3:" <<msg3->data << endl;
    cout << "msg4:" <<msg4->data << endl;
	...
	humans_msgs::Human h = msg1->human[i];
	h.body.is_speaked = msg3->data;
	msgs.push_back(h);
      }
    
    for(int i = 0; i<msg2->human.size(); ++i)
      {
	humans_msgs::Human h = msg2->human[i];
	h.body.is_speaked = msg4->data;
	msgs.push_back(h);
      }
    //msgs.push_back(msg2);
